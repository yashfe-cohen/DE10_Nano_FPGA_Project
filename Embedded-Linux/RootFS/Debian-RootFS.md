...
לפני שניכנס לסביבה החדשה, עלינו לחבר (**Mount**) את מערכות הקבצים הווירטואליות של המארח לתוך תיקיית היעד.

פעולה זו מאפשרת לחבילות הבסיס לגשת למשאבי המערכת הדרושים להמשך ההתקנה.
...
## כניסה לסביבת ה־chroot

כעת ניכנס לתוך התיקייה שיצרנו בעזרת הפקודה `chroot`.

פקודה זו גורמת למערכת להתייחס לתיקייה החדשה כאילו היא מערכת הקבצים הראשית (`root filesystem`) שלנו.
...
## הפעלת שלב ההתקנה השני

כעת, כאשר אנחנו נמצאים "בתוך" סביבת ה־ARM המדומה, נפעיל את השלב השני של ההתקנה:

```bash
/debootstrap/debootstrap --second-stage
```

תהליך זה אורך בערך **5 דקות**.

במהלך שלב זה, המערכת תתקין ותגדיר באופן סופי את כל חבילות הבסיס הדרושות לסביבת ה־ARM.

---

## יציאה מסביבת ה־chroot

לאחר שהתהליך יסתיים בהצלחה, נצא מהסביבה המדומה:
...
כעת ננתק בחזרה את מערכות הקבצים הווירטואליות שחיברנו:
...
לאחר סיום שלב זה, סביבת ה־ARM מוכנה להמשך תהליך הבנייה וההגדרות.
